...
def test_dry_run_preserves_file_system(files_data):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for filename, extension in files_data:
            ...
            src_file.write_text(
                f"content of {full_filename}", encoding="utf-8")
            created_files.append(src_file)

        # Record initial state
        initial_files = {
            f: f.read_text(encoding="utf-8") for f in created_files if f.exists()
        }
        initial_file_paths = set(initial_files.keys())

        # Check that category folders don't exist yet
        category_folders = [root / "img", root /
                            "vid", root / "arc", root / "msk"]
        for folder in category_folders:
            assert not folder.exists(), (
                f"Category folder {folder} should not exist before dry-run"
            )
        ...
        for file_path, original_content in initial_files.items():
            ...
            ), f"File {file_path} was removed during dry-run"
...
def test_dry_run_displays_operation_logs(files_data):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for full_filename, _ in created_files:
            ...
            assert "[DRY RUN]" in output, "Expected [DRY RUN] prefix in operation logs"
...
def test_dry_run_mode_indicators(files_data):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert "[DRY RUN]" in output, "Expected [DRY RUN] indicator in output"
