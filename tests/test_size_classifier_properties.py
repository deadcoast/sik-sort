...
@given(
    ...
    medium_size=st.integers(
        min_value=1_048_577, max_value=104_857_600
    ),  # 1 MB to 100 MB
    large_size=st.integers(
        min_value=104_857_601, max_value=209_715_200
    ),  # 100 MB to 200 MB
)
...
@settings(max_examples=100, deadline=None)
@given(
    small_threshold=st.integers(
        min_value=1024, max_value=5_242_880),  # 1 KB to 5 MB
    medium_threshold=st.integers(
        min_value=5_242_881, max_value=52_428_800
    ),  # 5 MB to 50 MB
    file_size=st.integers(
        min_value=0, max_value=104_857_600
    ),  # 0 to 100 MB (reduced for speed)
)
def test_custom_size_thresholds(small_threshold, medium_threshold, file_size):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        thresholds = SizeThresholds(
            small_max=small_threshold, medium_max=medium_threshold
        )
...
@given(file_size=st.integers(min_value=0, max_value=10_485_760))  # 0 to 10 MB
...
@settings(max_examples=100)
@given(size_bytes=st.integers(min_value=0, max_value=1_099_511_627_776))  # 0 to 1 TB
def test_format_size_validity(size_bytes):
    ...
    assert len(parts) == 2, (
        f"Formatted size should have format 'NUMBER UNIT', got '{formatted}'"
    )
