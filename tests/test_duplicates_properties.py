        ...
        for file_path in files:
            ...
            if algorithm == "md5":
                assert len(hash_value) == 32, (
                    f"MD5 hash should be 32 characters, got {len(hash_value)}"
                )
...
def test_duplicate_handling_correctness(
    unique_content, num_duplicates, num_unique, algorithm
):
    """
...
@given(
    ...
    duplicates_per_group=st.lists(
        st.integers(min_value=2, max_value=6), min_size=1, max_size=5
    ),
    file_size=st.integers(min_value=100, max_value=10240),
)
def test_duplicate_statistics_accuracy(
    num_duplicate_groups, duplicates_per_group, file_size
):
    """
