...
from sik_sort.sorter import EnhancedSortingStats, sort_files_with_date
...
timestamps = st.integers(
    min_value=int(time.time()) - (5 * 365 * 24 * 60 * 60), max_value=int(time.time())
)


@given(filename=filenames, timestamp=timestamps)
...
def test_date_hierarchy_structure(filename, timestamp):
    ...
    **Validates: Requirements 4.5**
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        expected_date_folder = classify_by_date(
            test_file, use_creation=False, date_format="%Y-%m"
        )

        # Sort files with date-based sorting
        stats = sort_files_with_date(
            source_path, use_creation=False, date_format="%Y-%m", dry_run=False
        )
        ...
        assert len(parts) >= 3, (
            f"Expected at least 3 path parts, got {len(parts)}: {parts}"
        )

        # First part should be date folder in YYYY-MM format
        date_part = parts[0]
        assert len(date_part) >= 7, (
            f"Date folder should be at least 7 chars (YYYY-MM), got {date_part}"
        )
        assert date_part == expected_date_folder, (
            f"Expected date folder {expected_date_folder}, got {date_part}"
        )
