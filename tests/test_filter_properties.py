...
def file_path_strategy(draw):
    ...
    extension = draw(
        st.sampled_from(
            [".jpg", ".png", ".pdf", ".txt", ".mp4", ".zip", ".doc", ".tmp", ".bak"]
        )
    )
...
def test_include_filter_correctness(files, pattern):
    ...
    for file in filtered:
        assert matches_pattern(file, pattern), (
            f"File {file} in filtered list but doesn't match pattern {pattern}"
        )

    # Verify no files that don't match the pattern are included
    for file in files:
        if file not in filtered:
            assert not matches_pattern(file, pattern), (
                f"File {file} matches pattern {pattern} but was excluded"
            )
...
def test_exclude_filter_correctness(files, pattern):
    ...
    for file in filtered:
        assert not matches_pattern(file, pattern), (
            f"File {file} matches exclude pattern {pattern} but was included"
        )
    ...
    for file in files:
        if matches_pattern(file, pattern):
            assert file not in filtered, (
                f"File {file} matches exclude pattern {pattern} but was not excluded"
            )
...
@given(
    files=st.lists(file_path_strategy(), min_size=1, max_size=50),
    extensions=st.sets(
        st.sampled_from([".jpg", ".png", ".pdf", ".txt", ".mp4"]),
        min_size=1,
        max_size=3,
    ),
)
def test_extension_filter_correctness(files, extensions):
    ...
    for file in filtered:
        assert matches_extensions(file, extensions), (
            f"File {file} in filtered list but doesn't have allowed extension"
        )

    # Verify no files with disallowed extensions are included
    for file in files:
        if file not in filtered:
            assert not matches_extensions(file, extensions), (
                f"File {file} has allowed extension but was excluded"
            )
...
def test_filter_precedence(files, include_pattern, exclude_pattern):
    ...
    config = FilterConfig(
        include_patterns=[include_pattern], exclude_patterns=[exclude_pattern]
    )
    ...
    assert set(filtered) == set(expected), (
        "Filter precedence not correct: include should be applied before exclude"
    )
...
@given(
    ...
    include_extensions=st.sets(
        st.sampled_from([".jpg", ".png", ".pdf", ".txt"]), min_size=0, max_size=3
    ),
    exclude_extensions=st.sets(
        st.sampled_from([".tmp", ".bak", ".zip"]), min_size=0, max_size=2
    ),
)
def test_filter_statistics_accuracy(
    files, include_patterns, exclude_patterns, include_extensions, exclude_extensions
):
    """
    ...
    assert len(filtered) <= len(
        files), "Filtered list has more files than original"
