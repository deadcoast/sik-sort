...
@given(
    filename=st.text(
        ...
        alphabet=st.characters(
            min_codepoint=32,
            max_codepoint=126,
            # Exclude invalid filename chars and Rich markup chars
...
def test_operation_log_contains_filename_and_category(filename, category):
    ...
    try:
        ...
        assert filename in output, (
            f"Filename '{filename}' not found in log output: {output}"
        )
...
@given(
    files=st.lists(
        st.tuples(
            ...
            # Use same category for consistency check
            st.just(FileCategory.IMAGE),
        ),
        min_size=2,
        max_size=10,
    )
...
def test_error_log_visual_distinction(
    filename, error_message, normal_filename, category
):
    """
    ...
    try:
        ...
        assert error_message in error_output, (
            f"Error message not in error log: {error_output}"
        )
    finally:
        # Restore original console
        logger_module.console = original_console
...
@settings(max_examples=100)
@given(file_count=st.integers(min_value=0, max_value=10000))
