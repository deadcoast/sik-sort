...
from sik_sort.sorter import SortingStats, sort_files
...
@given(
    dir_name=st.text(
        ...
        lambda x: x.upper() not in WINDOWS_RESERVED_NAMES
        and not x.endswith(".")
        and not x.endswith(" ")
    )
)
def test_category_folders_are_created(dir_name):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for folder_name in expected_folders:
            ...
            ), f"Category folder '{folder_name}' should exist"
            assert folder_path.is_dir(), (
                f"Category folder '{folder_name}' should be a directory"
            )
...
def test_statistics_accuracy(num_images, num_videos, num_archives, num_misc):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert (
            stats.img_count + stats.vid_count + stats.arc_count + stats.msk_count
            == stats.total_files
        ...
        actual_img_count = len(list(img_folder.glob("*"))
                               ) if img_folder.exists() else 0
        actual_vid_count = len(list(vid_folder.glob("*"))
                               ) if vid_folder.exists() else 0
        actual_arc_count = len(list(arc_folder.glob("*"))
                               ) if arc_folder.exists() else 0
        actual_msk_count = len(list(msk_folder.glob("*"))
                               ) if msk_folder.exists() else 0
