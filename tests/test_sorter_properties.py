...
def test_filename_preservation(filename, extension):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        ), f"File {full_filename} was not moved to destination"
        ...
        ), f"Source file {src_file} still exists after move"
...
def test_no_file_overwriting(filename, extension, original_content, new_content):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        renamed_files = [f for f in dest_dir.iterdir() if f.name !=
                         full_filename]
...
def test_image_files_sorted_correctly(filename, extension):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert stats.img_count >= 1, (
            f"Image count in stats is {stats.img_count}, expected >= 1"
        )
...
def test_video_files_sorted_correctly(filename, extension):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Assert statistics are correct
        assert stats.vid_count >= 1, (
            f"Video count in stats is {stats.vid_count}, expected >= 1"
        )
...
def test_archive_files_sorted_correctly(filename, extension):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert stats.arc_count >= 1, (
            f"Archive count in stats is {stats.arc_count}, expected >= 1"
        )
...
@given(
    ...
    extension=st.sampled_from(
        ["txt", "doc", "docx", "pdf", "xls", "xlsx", "ppt", "pptx", "csv", "json"]
    ),
)
def test_miscellaneous_files_sorted_correctly(filename, extension):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Assert statistics are correct
        assert stats.msk_count >= 1, (
            f"Miscellaneous count in stats is {stats.msk_count}, expected >= 1"
        )
...
def test_largest_file_correctness(files_data):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        expected_operations = {"img": [], "vid": [], "arc": [], "msk": []}
...
@settings(max_examples=100)
@given(size_bytes=st.integers(min_value=0, max_value=10**15))  # Up to petabytes
def test_size_formatting_correctness(size_bytes):
    ...
    assert isinstance(
        formatted, str), f"Expected string, got {type(formatted)}"
    ...
    if unit == "B":
        ...
        assert abs(number - expected) < 0.01, (
            f"Expected ~{expected:.2f} KB, got {number} KB"
        )
    elif unit == "MB":
        # Should be in range [1, 1024)
        ...
        assert abs(number - expected) < 0.01, (
            f"Expected ~{expected:.2f} MB, got {number} MB"
        )
    elif unit == "GB":
        # Should be in range [1, 1024)
        ...
        assert abs(number - expected) < 0.01, (
            f"Expected ~{expected:.2f} GB, got {number} GB"
        )
    elif unit == "TB":
        # Should be >= 1
        ...
        assert abs(number - expected) < 0.01, (
            f"Expected ~{expected:.2f} TB, got {number} TB"
        )
