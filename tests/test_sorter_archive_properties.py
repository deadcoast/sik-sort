...
from sik_sort.sorter import EnhancedSortingStats, sort_files_archive_mode
...
def test_custom_date_format_support(filename, timestamp, date_format):
    ...
    **Validates: Requirements 8.5**
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        stats = sort_files_archive_mode(
            source_path, use_creation=False, date_format=date_format, dry_run=False
        )
        ...
        if "/" not in date_format:
            ...
            for i, (actual, expected) in enumerate(
                zip(actual_date_parts, expected_parts)
            ):
