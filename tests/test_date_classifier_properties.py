...
def test_date_folder_naming_format(year, month, day):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert len(parts) == 2, (
            f"Date folder should have format YYYY-MM, got '{date_folder}'"
        )
        assert len(parts[0]) == 4, f"Year should be 4 digits, got '{parts[0]}'"
        assert len(
            parts[1]) == 2, f"Month should be 2 digits, got '{parts[1]}'"
...
def test_creation_date_usage(
    creation_year, creation_month, modification_year, modification_month
):
    """
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        modification_date = datetime(
            modification_year, modification_month, 15, 12, 0, 0
        )
        ...
        assert len(parts) == 2, (
            f"Creation date should have format YYYY-MM, got '{creation_result}'"
        )
        assert len(parts[0]) == 4, f"Year should be 4 digits, got '{parts[0]}'"
        assert len(
            parts[1]) == 2, f"Month should be 2 digits, got '{parts[1]}'"
...
def test_modification_date_usage(year, month, day):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert file_date.month == month, (
            f"Month should be {month}, got {file_date.month}"
        )
...
@given(
    ...
    month=st.integers(min_value=1, max_value=12),
    day=st.integers(min_value=1, max_value=28),
    format_string=st.sampled_from(
        ["%Y-%m", "%Y-%m-%d", "%Y/%m", "%Y%m", "%B %Y"]),
)
...
def test_get_file_date_validity(year, month, day):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert 1 <= file_date.day <= 31, f"Day should be 1-31, got {file_date.day}"
...
def test_custom_date_format(year, month, day):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        custom_format = "%Y-%m-%d"
        date_folder = classify_by_date(
            test_file, use_creation=False, date_format=custom_format
        )
