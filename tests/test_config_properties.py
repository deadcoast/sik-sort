...
def config_dict_strategy(draw):
    ...
    return {
        ...
        "filters": {
            ...
            "include_extensions": draw(
                st.lists(st.text(min_size=2, max_size=5), max_size=3)
            ),
            "exclude_extensions": draw(
                st.lists(st.text(min_size=2, max_size=5), max_size=3)
            ),
        },
        ...
        "size_thresholds": {
            ...
            "medium_max": draw(
                st.integers(min_value=10_000_001, max_value=1_000_000_000)
            ),
        },
...
def test_configuration_loading_yaml(config_data):
    ...
    try:
        ...
        assert (
            ...
            == config_data["filters"]["include_patterns"]
        )
        assert (
            config.filters.exclude_patterns
            == config_data["filters"]["exclude_patterns"]
        )
        assert config.filters.include_extensions == set(
            config_data["filters"]["include_extensions"]
        )
        assert config.filters.exclude_extensions == set(
            config_data["filters"]["exclude_extensions"]
        )

        # Verify size settings
        assert config.size_sorting_enabled == config_data["size_sorting_enabled"]
        assert (
            config.size_thresholds.small_max
            == config_data["size_thresholds"]["small_max"]
        )
        assert (
            config.size_thresholds.medium_max
            == config_data["size_thresholds"]["medium_max"]
        )
        ...
        expected_mode = (
            DateMode.CREATION
            if config_data["date_mode"] == "creation"
            else DateMode.MODIFICATION
        )
        ...
        assert (
            config.duplicate_detection_enabled
            == config_data["duplicate_detection_enabled"]
        )
...
def test_configuration_completeness(config_data):
    ...
    try:
        ...
        assert len(config.filters.include_patterns) == len(
            config_data["filters"]["include_patterns"]
        )
        assert len(config.filters.exclude_patterns) == len(
            config_data["filters"]["exclude_patterns"]
        )
...
def cli_args_strategy(draw):
    ...
    return {
        ...
        "size_medium_max": draw(
            st.integers(min_value=5_000_001, max_value=500_000_000)
        ),
        "date_sorting_enabled": draw(st.booleans()),
        ...
        "duplicate_detection_enabled": draw(st.booleans()),
        "hash_algorithm": draw(
            st.sampled_from(["sha256"])
        ),  # Different from config default
        "archive_mode": draw(st.booleans()),
        "report_enabled": draw(st.booleans()),
        "report_format": draw(
            st.sampled_from(["csv"])
...
@settings(max_examples=100)
@given(config_data=config_dict_strategy(), cli_args=cli_args_strategy())
def test_cli_argument_precedence(config_data, cli_args):
    ...
    try:
        ...
        assert (
            merged_config.duplicate_detection_enabled
            == cli_args["duplicate_detection_enabled"]
        )
...
def test_custom_category_names(custom_names):
    ...
    """
    config_data = {"custom_categories": custom_names}
...
def test_custom_extension_mappings(custom_extensions):
    ...
    """
    config_data = {"custom_extensions": custom_extensions}
...
def test_auto_cleanup_behavior(auto_cleanup):
    ...
    """
    config_data = {"auto_cleanup": auto_cleanup}
