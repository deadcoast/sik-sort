...
def _dict_to_config(data: dict[str, Any]) -> Config:
    ...
    if "date_mode" in data:
        ...
        config.date_mode = (
            DateMode.CREATION if mode_str == "creation" else DateMode.MODIFICATION
        )
    ...
    if "duplicate_detection_enabled" in data:
        config.duplicate_detection_enabled = bool(
...
def validate_config(config: Config) -> list[str]:
    ...
    if config.hash_algorithm not in ["md5", "sha256"]:
        errors.append(
            f"hash_algorithm must be 'md5' or 'sha256', got '{config.hash_algorithm}'"
        )

    # Validate report format
    if config.report_format not in ["json", "csv"]:
        errors.append(
            f"report_format must be 'json' or 'csv', got '{config.report_format}'"
        )
