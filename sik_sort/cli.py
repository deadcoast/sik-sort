...
def parse_arguments() -> tuple[Optional[Path], bool]:
    ...
    parser = argparse.ArgumentParser(
        prog="sik", description="Sik Sort - Organize files into categorized folders"
    )
...
def prompt_for_path() -> Path:
    ...
    while True:
        path_str = Prompt.ask(
            "[bold cyan]Enter the source directory path[/bold cyan]")
        path = Path(path_str)
...
def confirm_cleanup() -> bool:
    ...
    """
    return Confirm.ask(
        "[bold yellow]Do you want to clean up empty folders?[/bold yellow]"
    )
...
def display_safety_warnings(warnings: list[str]) -> bool:
    ...
    console.print()
    console.print(
        "[bold yellow]Sorting this directory may reorganize important development files![/bold yellow]"
    )
    console.print(
        "[yellow]This could break your project or make files difficult to find.[/yellow]"
    )
    console.print()
...
def display_dry_run_banner() -> None:
    ...
    console.print(
        "[bold yellow]           No files will be modified                       [/bold yellow]"
    )
    console.print("[bold yellow]" + "=" * 60 + "[/bold yellow]")
    console.print()
