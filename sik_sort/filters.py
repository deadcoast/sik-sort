...
def apply_filters(files: list[Path], config: FilterConfig) -> tuple[list[Path], int]:
    ...
    if config.include_patterns:
        filtered = [
            f
            for f in filtered
            if any(matches_pattern(f, pattern) for pattern in config.include_patterns)
        ]

    # Apply include extensions (if any)
    if config.include_extensions:
        filtered = [
            f for f in filtered if matches_extensions(f, config.include_extensions)
        ]
    ...
    if config.exclude_patterns:
        filtered = [
            ...
            for f in filtered
            if not any(
                matches_pattern(f, pattern) for pattern in config.exclude_patterns
            )
        ]
    ...
    if config.exclude_extensions:
        filtered = [
            f for f in filtered if not matches_extensions(f, config.exclude_extensions)
        ]
