...
from .date_classifier import DateMode, classify_by_date
from .duplicates import calculate_space_saved, find_duplicates
...
def sort_files(
...
) -> Union[SortingStats, EnhancedSortingStats]:
    ...
    for i, file_path in enumerate(files, 1):
        ...
        actual_dest = move_file_with_conflict_resolution(
            file_path, dest_path, dry_run=dry_run
        )
    ...
    for i, file_path in enumerate(files, 1):
        try:
            ...
            date_folder = classify_by_date(
                file_path, use_creation, date_format)
            type_category = classify_file(file_path)
    ...
    for i, file_path in enumerate(files, 1):
        try:
            ...
            if with_type_hierarchy:
                ...
                type_category = classify_file(
                    file_path)  # Still classify for stats
    ...
    for i, file_path in enumerate(files, 1):
        try:
            if file_path in duplicate_files:
                ...
                dest_path = (
                    dest_folder /
                    f"{file_path.stem}_duplicate{file_path.suffix}"
                )
