    ...
    try:
        ...
        if warnings:
            if not display_safety_warnings(warnings):
                console.print(
                    "[yellow]Operation cancelled for safety.[/yellow]")
                return
        ...
        stats = sort_files(
            source_path, dry_run=dry_run, ascii_progress_callback=display_ascii_progress
        )
        ...
        if not dry_run:
            ...
            if confirm_cleanup():
                ...
                console.print(
                    f"[green]Removed {removed_count} empty directories.[/green]"
                )
            else:
        ...
        if dry_run:
            ...
            console.print(
                "[bold yellow]           No changes were made                          [/bold yellow]"
            )
            console.print("[bold yellow]" + "=" * 60 + "[/bold yellow]")
